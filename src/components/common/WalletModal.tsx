...
const walletInfos: WalletInfos = {
    ...
    "coinbaseWallet": {
        text: "Connect to your Coinbase Wallet",
...
const WalletModal = () => {
    ...
    const { isShowWalletModal } = useSelector((state: any) => state.data);
    const { hideWalletModal } = useWallet();

    return <Modal size='small' show={(isShowWalletModal && !isConnected)} onClose={() => hideWalletModal()}>
        <div className=''>
            {
                connectors.map((connector) => (
                    <div
                        className='w-full flex flex-col items-center p-4 cursor-pointer text-white bg-bg-pattern hover:bg-body-primary transition-all'
                        // disabled={!connector.ready}
                        key={connector.id}
                        onClick={() => connect({ connector })}
                    >
                        <img className='w-[56px] h-[56px]' src={walletInfos[connector.id].icon} alt={connector.id} />
                        <h4 className='my-2 text-lg font-bold'>{connector.name}</h4>
                        <p>{walletInfos[connector.id].text}</p>
